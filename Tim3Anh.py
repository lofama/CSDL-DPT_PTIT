...
#   img_hsv=covert_image_rgb_to_hsv(img)
  ...
  # embedding[0]=0
    ...
    combined_feature_vectors_hsv_hog = np.load("concat_hog_rgb.npy")
    if has :
       combined_feature_vectors_hsv_hog=combined_feature_vectors_hsv_hog.append(array_concat_hog_hsv)
    ...
    if has :
       combined_feature_vectors_rgb_hog=combined_feature_vectors_rgb_hog.append(array_concat_hog_rgb)
    ...
    for i, index in enumerate(closest_images_indices_rgb_hog):
        ...
        print("Ảnh", i+1, ":", index, "- Phần trăm giống:", similarity_percent)
        listImg.append([i,load_y[index],similarity_percent])
    return listImg 
def display_images(file_path):
    # Load the image
    target_image_index = 136
    load_y = np.load("y.npy",allow_pickle=True)
    listIMG = []
    ...
    # Display image name
    image_name_label1.config(text=listIMG[0][1])
    image_name_label2.config(text=listIMG[1][1])
    image_name_label3.config(text=listIMG[2][1])
